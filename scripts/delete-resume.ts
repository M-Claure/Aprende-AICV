 ...
 *
 * It exists because there is no delete path in the product: a job seeker has no
 * account and deliberately no recovery flow, so nothing in the UI can retire a
 * résumé, and testing the funnel accumulates rows and private-bucket PDFs with no
 * way to clear them.
 *
 * ── What "every trace" means ────────────────────────────────────────────────
 * Deleting the `funnel` row cascades in Postgres to `iteration_1..3`. Two things
 * do NOT cascade and are therefore done explicitly, in this order:
 *
 ...
 *
 * `lib/env.ts` is deliberately NOT used: it is `server-only`, it validates the
 * Azure and Amplitude configuration this script does not need, and a project
 * with a stale AI key must still be cleanable.
 */
...
async function collectDetail(admin: SupabaseClient, row: FunnelRow): Promise<Detail> {
  ...
  return { row, iterations, storagePaths: await listStoragePaths(admin, row), spend };
}

/**
 * The PDFs actually in the bucket for this résumé.
 *
 * Listed rather than assumed: the canonical five names (`resumePdfPath` for
 * stages 0–3, plus the English translation) are what the app writes today, but an
 * object written under a name this script does not know about would survive a
 * delete-by-name and stay in a private bucket forever. The canonical paths are
 * added as a floor in case the list call fails.
 *
 * Listing the folder is also why the English PDF needs no special case here: it
 * deliberately lives beside the Spanish ones rather than in a sibling folder.
 */
...
function printTable(details: Detail[], numbered: boolean): void {
  ...
  const header =
    ...
    `${pad("cv", 8)}${pad("pdf", 5)}${pad("rondas", 8)}${pad("gasto", 9)}creado`;
  console.log(header);
...
function printPlan(details: Detail[], args: Args): void {
  ...
  if (details.every((d) => !d.spend.available)) {
    console.log("\nEste proyecto no tiene las tablas de 0009_usage_limits.sql, así que no hay gasto ni contadores que limpiar.");
  }
}
...
async function deleteProfile(admin: SupabaseClient, d: Detail, args: Args): Promise<void> {
  ...
  // 3. The row. Cascades to iteration_1..3.
  ...
  console.log(`  ✓ ${tag} — ${d.storagePaths.length} PDF(s), ${d.iterations} ronda(s)`);
}
 ...
 *
 * The guard exists because deleting an `auth.users` row cascades to every
 * `funnel` row it owns: a user with résumés we were not asked about would lose
 * them silently.
 *
 * There is no employer-account check here because this repository has no talent
 * directory — every `auth.users` row is a job seeker's guest identity. If the
 * directory is ever ported over, add one: an employer account is a real
 * credential and must not be collateral of a résumé wipe.
 */
