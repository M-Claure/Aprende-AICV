...
describe("processAnswer — adaptive flow (spec §17 scenario)", () => {
  it("walks a family-business user through the script to a ready profile", async () => {
    ...
    // 3. The job being sought. The desired position lands in targetRole; the
    ...
    // 4. Education — one question, then the funnel moves on.
    ...
    // 5. How many experiences — creates one still-empty entry per experience.
    ...
    // 6. The skills the person names themselves — the funnel's only source of
    ...
    // 7. One description per experience: raw wording preserved, type kept.
    res = await answer(
      "experience_add",
      "experience",
